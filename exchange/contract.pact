...
(module myswap-exchange GOVERNANCE
  ...
  (defun init ()
    (with-capability (ISSUING)
      (myswap-exchange-tokens.init-issuer (create-issuing-guard)))
  )
    ...
    (let*
      ...
      (let*
        ( (token0:module{fungible-v2} (at 'token (at 'leg0 p)))
          ...
          (totalSupply (myswap-exchange-tokens.total-supply key))
          (liquidity (myswap-exchange-tokens.truncate key
            (if (= totalSupply 0.0)
              (with-capability (ISSUING)
        ...
        { "liquidity": liquidity
        , "supply": (myswap-exchange-tokens.total-supply key)
        , "amount0": amount0
        , "amount1": amount1
        }
      )
  ...
  (defun mint (token:string to:string guard:guard amount:decimal)
    (require-capability (ISSUING))
    (install-capability (myswap-exchange-tokens.MINT token to amount))
    (myswap-exchange-tokens.mint token to guard amount)
  )
  ...
  (defun remove-liquidity:object
    ...
    (let* ( (p (get-pair tokenA tokenB))
      ...
      (myswap-exchange-tokens.transfer pair-key sender pair-account liquidity)
      (let*
        ( (token0:module{fungible-v2} (at 'token (at 'leg0 p)))
          (token1:module{fungible-v2} (at 'token (at 'leg1 p)))
          (balance0 (token0::get-balance pair-account))
          (balance1 (token1::get-balance pair-account))
          (liquidity_ (myswap-exchange-tokens.get-balance pair-key pair-account))
          (total-supply (myswap-exchange-tokens.total-supply pair-key))
  ...
  (defun burn (token:string to:string amount:decimal)
    (require-capability (ISSUING))
    (install-capability (myswap-exchange-tokens.BURN token to amount))
    (myswap-exchange-tokens.burn token to amount)
  )
  ...
  (defun swap-alloc
    ...
    (let*
      ...
      (swap myswap-noop-callable recipient recip-guard
        (at 'token-out alloc)
        (at 'out alloc)
        (at 'token-in alloc)))
  )
  ...
  (defun swap
    ( callable:module{myswap-callable-v1}
      recipient:string
      recip-guard:guard
      token:module{fungible-v2}
      amount-out:decimal
  ...
  (defun create-pair
    ...
    \ in 'myswap-exchange-tokens' module, and new empty accounts in each leg token. \
    ...
    (let* ((key (get-pair-key token0 token1))
      ...
      (with-capability (CREATE_PAIR ctoken0 ctoken1 key a)
        ...
        (myswap-exchange-tokens.create-account key a lpg)
        { "key": key
        , "account": a
        }))
    )
